...
def __usage(prompt=None):
    if prompt:
        print(prompt)
    tip_str = '''请在$HOME根目录下创建.sman_config文件，并配置需要的值。示例：
...
def get_jumper_option(option_name, default_value=None):
    ...
    if not path.exists(sman_cfg_path) or not path.isfile(sman_cfg_path):
        __usage('配置文件：%s 不存在或不是文件' % sman_cfg_path)
    cf.read(sman_cfg_path)

    if not cf.has_section(jumper_section):
        __usage('配置文件：%s 配置域缺失：%s' % (sman_cfg_path, jumper_section))

    return __get_option(sman_cfg_path, cf, jumper_section, option_name, default_value)
...
def set_jumper_option(option_name, value):
    ...
    # print('配置文件：%s 配置域：%s 配置项：%s=%s 已设置' % (sman_cfg_path, jumper_section, option_name, value))
